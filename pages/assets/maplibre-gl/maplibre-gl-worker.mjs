...
var U = class {
    ...
    update(e, t, n) {
      for (let t of e) {
        ...
        let e = (this._layers[t.id] = c(t, n));
        ((e._featureFilter = C(e.filter, `layers[${t.id}].filter`, n)),
      ...
      let r = te(Object.values(this._layerConfigs), this.keyCache);
      for (let e of r) {
...
var G = class {
  ...
  async parse(e, t, r, i, a) {
    ((this.status = `parsing`),
      (this.data = e),
      (this.collisionBoxArray = new j()));
    let o = new w(Object.keys(e.layers).sort()),
      c = new S(this.tileID, this.promoteId);
    c.bucketLayerIDs = [];
    let l = {},
      u = {
        featureIndex: c,
      ...
      f = t.familiesBySource[this.source];
    for (let t in f) {
      let i = e.layers[t];
      if (!i) continue;
      i.version === 1 &&
      ...
      for (let e = 0; e < i.length; e++) {
        let n = i.feature(e),
          r = c.getId(n, t);
        s.push({ feature: n, id: r, index: e, sourceLayerIndex: a });
      }
      for (let e of f[t]) {
        let t = e[0];
        (t.source !== this.source &&
          ...
          !t.isHidden(this.zoom, !0) &&
            ...
            c.bucketLayerIDs.push(e.map((e) => e.id))));
      }
    }
    let m = ee(u.glyphDependencies, (e) => Object.keys(e).map(Number));
    ...
    let g = Object.keys(u.iconDependencies),
      _ = Promise.resolve({});
    if (g.length) {
      let e = new AbortController();
      (this.inFlightDependencies.push(e),
    ...
    let v = Object.keys(u.patternDependencies),
      y = Promise.resolve({});
    if (v.length) {
      let e = new AbortController();
      (this.inFlightDependencies.push(e),
    ...
    let b = u.dashDependencies,
      x = Promise.resolve({});
    if (Object.keys(b).length) {
      let e = new AbortController();
      (this.inFlightDependencies.push(e),
        (x = i.sendAsync({ type: `GDA`, data: { dashes: b } }, e)));
    }
    let [C, T, E, D] = await Promise.all([h, _, y, x]),
      k = new W(C),
      A = new p(T, E);
    for (let e in l) {
      let t = l[e];
      t instanceof s
        ? (K(t.layers, this.zoom, r),
          ne({
            bucket: t,
            glyphMap: C,
            glyphPositions: k.positions,
            imageMap: T,
            imagePositions: A.iconPositions,
            showCollisionBoxes: this.showCollisionBoxes,
            canonical: this.tileID.canonical,
            subdivisionGranularity: u.subdivisionGranularity,
          }))
        : t.hasDependencies &&
          (t instanceof O || t instanceof R || t instanceof d) &&
          (K(t.layers, this.zoom, r),
          t.addFeatures(u, this.tileID.canonical, A.patternPositions, D));
    }
    return (
      (this.status = `done`),
      {
        buckets: Object.values(l).filter((e) => !e.isEmpty()),
        featureIndex: c,
        collisionBoxArray: this.collisionBoxArray,
        glyphAtlasImage: k.image,
        imageAtlas: A,
        dashPositions: D,
        glyphMap: this.returnDependencies ? C : null,
        iconMap: this.returnDependencies ? T : null,
        glyphPositions: this.returnDependencies ? k.positions : null,
      }
    );
  }
};
...
var q = class {
  ...
  Y = class {
    ...
    loadGeometry() {
      return this.pointsArray.map((e) => e.map((e) => new A(e.x, e.y)));
    }
  },
  X = class {
    constructor(e, t, n) {
...
function ie(e, t, n) {
  ...
  for (let t = 0; t < e.length; t++) {
    ...
    ((c = E(c, n.type, -128, -128, r + 128, r + 128)),
      c.length !== 0 && s.push(new Y(n.type, c, n.properties, n.id, r)));
  }
  return new X(s, e.name, r);
}
var ae = class {
    constructor(e, t, n) {
      ((this.actor = e),
        ...
        (this.tileState = new q()),
        (this.overzoomedTileResultCache = new F(1e3)));
    }
    loadVectorTile(e, t) {
      try {
        return {
          vectorTile: e.encoding === `mlt` ? new M(t) : new P(new m(t)),
          rawData: t,
        };
      } catch (n) {
        ...
        throw (
          i
            ? (a += `please make sure the data is not gzipped and that you have configured the relevant header in the server`)
            : (a += `got error: ${h(n).message}`),
          Error(a)
        );
      }
    }
    async loadTile(e) {
      ...
      try {
        ...
        try {
          return await this._parseWorkerTile(i, e, f);
        } finally {
          this.tileState.removeParsing(t);
        }
      } catch (e) {
        throw (
          this.tileState.finishLoading(t),
    ...
    _getEtagUnmodifiedResult(e, t) {
      return _(
        { etagUnmodified: !0 },
        this._getExpiryData(e),
        this._finishRequestTiming(t),
      );
    }
    async _parseWorkerTile(e, t, n) {
      let r = await e.parse(
        e.vectorTile,
        this.layerIndex,
      ...
      if (n) {
        let { rawData: e, cacheControl: i, resourceTiming: a } = n,
          o = t.overzoomParameters ? `mvt` : t.encoding;
        r = _({ rawTileData: e.slice(0), encoding: o }, r, i, a);
      }
      return r;
    }
    ...
    _startRequestTiming(e) {
      if (e.request?.collectResourceTiming) return new J(e.request.url);
    }
    ...
    _getOverzoomTile(e, t) {
      ...
      let u = { vectorTile: c, rawData: N(c).buffer };
    ...
    async reloadTile(e) {
      ...
      if (
        ((n.showCollisionBoxes = e.showCollisionBoxes), n.status === `parsing`)
      ) {
        let r = this.tileState.getParsing(t);
        try {
          return await this._parseWorkerTile(n, e, r);
        } finally {
          this.tileState.removeParsing(t);
        }
      }
      if (n.status === `done` && n.vectorTile)
        return await this._parseWorkerTile(n, e);
    }
    async abortTile(e) {
  ...
  oe = class {
    ...
    async loadTile(e) {
      let {
        ...
        u = new B(
          t,
          x(r) ? new b({ width: c, height: l }, await L(r, -1, -1, c, l)) : r,
          n,
          i,
          a,
          o,
      ...
      return ((this.loaded ||= {}), (this.loaded[t] = u), u);
    }
    removeTile(e) {
      let t = this.loaded,
        n = e.uid;
  ...
  se = class {
    ...
    loadVectorTile(e) {
      ...
      let { z: n, x: r, y: i } = e.tileID.canonical,
        a = this._geoJSONIndex.getTile(n, r, i);
      if (!a) return null;
      let o = new I(a.features, { version: 2, extent: u });
      return { vectorTile: o, rawData: N(o, t).buffer };
    }
    async loadTile(e) {
      let { uid: t } = e,
        n = new G(e);
      ...
      try {
        ...
        try {
          return await this._parseWorkerTile(n, e, o);
        } finally {
          this.tileState.removeParsing(t);
        }
      } catch (e) {
        throw ((n.status = `done`), this.tileState.markLoaded(t, n), e);
      }
    }
    async _reloadLoadedTile(e) {
      let t = e.uid,
        n = this.tileState.getLoaded(t);
      if (!n)
        throw Error(
    ...
    async _parseWorkerTile(e, t, n) {
      ...
      if (n) {
        let { rawData: e } = n;
        r = _({ rawTileData: e.slice(0), encoding: `mvt` }, r);
      }
      return r;
    }
    async abortTile(e) {
    ...
    reloadTile(e) {
      return this.tileState.getLoaded(e.uid)
        ? this._reloadLoadedTile(e)
        : this.loadTile(e);
    }
    async loadAndProcessGeoJSON(e, t) {
      if ((e.request && (e.data = (await V(e.request, t)).data), e.data)) {
...
function ce(t, n) {
  return new e(
    t,
    _(n.geojsonVtOptions || {}, { updateable: !0, clusterOptions: Z(n) }),
  );
}
...
var $ = class {
  constructor(e) {
    ((this.self = e),
      ...
      (this.self.addProtocol = r),
      (this.self.removeProtocol = f),
      (this.self.registerRTLTextPlugin = (e) => {
        o.setMethods(e);
      }),
      (this.self.makeRequest = H),
  ...
  _getLayerIndex(e) {
    ...
    return ((t ||= this.layerIndexes[e] = new U()), t);
  }
